...
class Skeleton(pg.sprite.Sprite):
    def __init__(self, x, y, platform_group, scroll):
        ...
        # Load sprite image
        self.image = pg.image.load("assets/enemies/skeleton/skeleton.png").convert_alpha()
        ...
        self.vertical_speed = 0  # Initial vertical speed for gravity
        self.gravity = 0.5  # Adjust this value as needed for gravity effect
        ...
        self.health_bar_offset_y = self.rect.height / 2 - 35  # 10 pixels above the center of the sprite
        ...
        self.hit_sound = pg.mixer.Sound("assets/soundeffects/skeletonhit.mp3")
    ...
    def update(self, player):
        ...
        # Despawn the skeleton if its health reaches 0
        if self.health <= 0:
            self.kill()  # Remove the Skeleton sprite from the group
    ...
    def decrease_health(self, amount):
        # Check if the skeleton is currently invincible
        if not self.invincible:
            self.health -= amount
            if self.health <= 0:
                self.kill()  # Despawn the skeleton if its health reaches 0
            # Set the skeleton to be invincible and record the time of the hit
    ...
    def check_invincibility(self):
        # Check if the skeleton is currently invincible and if the invincibility duration has elapsed
